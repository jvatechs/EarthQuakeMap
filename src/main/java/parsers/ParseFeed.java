...
//            float depthVal = getFloatVal(xml);
			...
			if (titleStr != null) {
				...
				Pattern pMag = Pattern.compile("M\\s*(\\d+(\\.\\d+)?)");
				Matcher m = pMag.matcher(titleStr);
				if (m.find()) {
					point.putProperty("magnitude", Float.parseFloat(m.group(1)));
				}
			}

			float depthVal = getFloatVal(xml);
			point.putProperty("depth", Math.abs(depthVal / 1000f));
